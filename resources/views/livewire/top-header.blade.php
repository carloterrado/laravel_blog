<header class="px-2 md:px-12 py-2 md:py-4 sticky top-0 left-0 bg-white shadow">
    <nav class="flex gap-6 items-center">
        ...
        <div class="ml-auto font-semibold relative ">
            <button id="profile" data-dropdown-toggle="logout" data-dropdown-placement="bottom-start" type="button">
                <div class="flex items-center space-x-4">
                    <img class="w-10 h-10 rounded-full object-cover" src="{{ $author->picture }}" alt="">
                    <div class="font-normal dark:text-white text-sm">
                        <div class="text-left">{{ $author->username }}</div>
                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ $author->name }}</div>
                    </div>
                </div>
            </button type="button">
                ...
                <ul class="py-2" aria-labelledby="user-menu-button">
                    ...
                    <li>
                        <form action="{{ route('author.logout') }}" method="post">
                            @csrf
                            <button type="submit"
                                class="block px-4 w-full text-left py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white ">logout</button>
                        </form>
                    </li>
                </ul>
            </div>
